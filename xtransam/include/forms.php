...
//  ------------------------------------------------------------------------ //
	function languagesForm_display() {
		...
		$ele_tray[0]->addElement(new XoopsFormText(_AM_XTRANSAM_NAME, 'name[0]', 15, 255));		
		$ele_tray[0]->addElement(new XoopsFormText(_AM_XTRANSAM_CODE, 'code[0]', 10, 6));
		$ele_tray[0]->addElement(new XoopsFormText(_AM_XTRANSAM_FOLDER, 'folder[0]', 15, 255));
		...
		foreach ($langs as $lang) {
			...
			$ele_tray[$lang->getVar('lang_id')]->addElement(new XoopsFormText(_AM_XTRANSAM_NAME, 'name['.$lang->getVar('lang_id').']', 15, 255, htmlspecialchars($lang->getVar('name'))));
			...
			$ele_tray[$lang->getVar('lang_id')]->addElement(new XoopsFormText(_AM_XTRANSAM_FOLDER, 'folder['.$lang->getVar('lang_id').']', 15, 255, htmlspecialchars($lang->getVar('foldername'))));
	...
	function translationForm_display($display=false)
	{
		...
		foreach ($ios as $io) {
		    $gio_id = $io->getVar('id');
			$ele_tray[$gio_id] = new XoopsFormElementTray($io->getVar('point'),'&nbsp;');
			$ele_tray[$gio_id]->addElement(new XoopsFormLabel(_AM_XTRANSAM_LABELTRANSLATEPATH, $io->getVar('path')));
			$ele_tray[$gio_id]->addElement(new XoopsFormLabel(_AM_XTRANSAM_LABELTRANSLATEPOINT, $io->getVar('point')));
			$lang_from = $lang_handler->get($io->getVar('languagefrom'));
			$lang_to = $lang_handler->get($io->getVar('languageto'));			
			$ele_tray[$gio_id]->addElement(new XoopsFormLabel(_AM_XTRANSAM_LABELTRANSLATEFROM, $lang_from->getVar('name')));
			$ele_tray[$gio_id]->addElement(new XoopsFormLabel(_AM_XTRANSAM_LABELTRANSLATETO, $lang_to->getVar('name')));
			$ele_tray[$gio_id]->addElement(new XoopsFormLabel('&nbsp;|&nbsp;', '<a href="index.php?op=deletebuffer&id='.$gio_id.'"><img src='. $pathImageIcon ."/delete.png alt="._DELETE.'" title="'._DELETE.'" /></a>'));
			if ($display==false) {
				$criteria = new CriteriaCompo(new Criteria('ioid', $gio_id));
				if ($files_handler->getCount($criteria)==0) {
					$ele_tray[$gio_id]->addElement(new XoopsFormLabel('&nbsp;|&nbsp;', '<a href="index.php?op=analysis&id='.$gio_id.'"><img src='. $pathImageIcon ."/document_analysis.png alt="._AM_XTRANSAM_ANALYSIS.'" title="'._AM_XTRANSAM_ANALYSIS.'" /></a>'));
				} else {
					$criteria->add(new Criteria('imported', '0'));			
					if ($files_handler->getCount($criteria)>0) {
						$ele_tray[$gio_id]->addElement(new XoopsFormLabel('&nbsp;|&nbsp;', '<a href="index.php?op=import&id='.$gio_id.'"><img src='. $pathImageIcon ."/document_import.png alt="._AM_XTRANSAM_IMPORT.'" title="'._AM_XTRANSAM_IMPORT.'" /></a>'));
					} else {
						$criteria = new CriteriaCompo(new Criteria('ioid', $gio_id));
						$criteria->add(new Criteria('auto', '1'));									
						if ($trans_handler->getCount($criteria)!=$trans_handler->getCount(new Criteria('ioid', $gio_id))) {
							$ele_tray[$gio_id]->addElement(new XoopsFormLabel('&nbsp;|&nbsp;', '<a href="index.php?op=translate&id='.$gio_id.'"><img src='. $pathImageIcon ."/document_traslate.png alt="._AM_XTRANSAM_TRANSLATE.'" title="'._AM_XTRANSAM_TRANSLATE.'" /></a>'));				
						}
						$ele_tray[$gio_id]->addElement(new XoopsFormLabel('&nbsp;|&nbsp;', '<a href="index.php?op=export&id='.$gio_id.'"><img src='. $pathImageIcon ."/document_export.png alt="._AM_XTRANSAM_EXPORT.'" title="'._AM_XTRANSAM_EXPORT.'" /></a>'));						
						$ele_tray[$gio_id]->addElement(new XoopsFormLabel('&nbsp;|&nbsp;', '<a href="index.php?op=manage&id='.$gio_id.'"><img src='. $pathImageIcon ."/document_edit.png alt="._AM_XTRANSAM_TRANSMANAGER.'" title="'._AM_XTRANSAM_TRANSMANAGER.'" /></a>'));				
					}
				}
			}
			$form_sel->addElement($ele_tray[$gio_id]);
		}
