		...
		if (strlen($value)<(5*1024)) {
			...
			if (count($response['error']['errors'])>0) {
				switch($response['code']) {
					case 403:
						XoopsCache::write('xtransam_google_pause', array('code'=>$response['code'], 'message'=>$response['message']), 60*60*24);
						break;
					default:
						XoopsCache::write('xtransam_google_pause', array('code'=>$response['code'], 'message'=>$response['message']), 60);
						break;
				}
				return '';
			} else {
				return parent::_unescapeUTF8EscapeSeq(parent::clean($response['data']['translations']['translatedText']));
			}
		}
	}
