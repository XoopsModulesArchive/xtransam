	...
	if ($trans_handler->getCount($criteria)>0) {
		...
		foreach($trans as $tran) {
			if ($tran->isempty()){
				...
				if ($start+$GLOBAL['xoopsModuleConfig']['php_execute_for']-3<time())
				{	
					$restart++;
					redirect_header("index.php?op=translate&id=$id&restart=$restart",2,sprintf(_AM_XTRANSAM_30SECONDSCONTINUING, $restart));			
				}
			}
		}
	}
