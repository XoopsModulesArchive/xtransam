...
class XtransamMymemoryProvider extends XtransamProviderHandler
{
	...
	function translate($fromcode, $tocode, $value)
	{
		...
		$response = parent::json_decode($this->send_curl($this->url, $value, $fromcode, $tocode, XOOPS_URL));
		if ($response['responseDetails']['responseStatus']==200) {
			return self::_unescapeUTF8EscapeSeq($this->clean($response['responseData']['translatedText']));
		} else { 
			return false;
		}
	}
