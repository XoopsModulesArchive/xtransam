...
case "translate":
	xoops_cp_header();
	echo sprintf(_AM_XTRANSAM_TRANSLATION_IN_PROCESS, $GLOBALS['xoopsModuleConfig']['php_execute_for'], isset($restart)?$restart:1);
	include "admin_footer.php";
	$io_handler = xoops_getmodulehandler('iobase', 'xtransam');
	$io = $io_handler->get($id);
	$trans_handler = xoops_getmodulehandler('translator', 'xtransam');
	...
	if ($trans_handler->getCount($criteria)>0) {
		...
		foreach($trans as $tran) {
			if ($tran->isempty()){
				...
				if ($start+$GLOBALS['xoopsModuleConfig']['php_execute_for']-3<time())
				{	
					$restart++;
					redirect_header("wizard.php?op=translate&id=$id&restart=$restart");
					exit(0);			
				}
			}
		}
...
default:
	...
	$count_traslactions = $traslactionsHandler->getCount($criteria);

 	$indexAdmin = new ModuleAdmin();	
	
    $indexAdmin->addInfoBox(_AM_XTRANSAM_ADMIN_NUMTRASL);
    $indexAdmin->addInfoBoxLine(_AM_XTRANSAM_ADMIN_NUMTRASL, "<label>"._AM_XTRANSAM_THEREARE_NUMTRASL."</label>", $count_traslactions, 'Green');
