...
//  ------------------------------------------------------------------------ //
				...
				if ($files_handler->getCount($criteria)==0) {
					...
					if ($files_handler->getCount($criteria)>0) {
						...
						if ($trans_handler->getCount($criteria)!=$trans_handler->getCount(new Criteria('ioid', $io->getVar('id')))) {
							$ele_tray[$io->getVar('id')]->addElement(new XoopsFormLabel('&nbsp;|&nbsp;', '<a href="index.php?op=translate&id='.$io->getVar('id').'">'._XT_TRANSLATE.'</a>'));				
						} else {
							$ele_tray[$io->getVar('id')]->addElement(new XoopsFormLabel('&nbsp;|&nbsp;', '<a href="index.php?op=export&id='.$io->getVar('id').'">'._XT_EXPORT.'</a>'));				
						}
						$ele_tray[$io->getVar('id')]->addElement(new XoopsFormLabel('&nbsp;|&nbsp;', '<a href="index.php?op=manage&id='.$io->getVar('id').'">'._XT_TRANSMANAGER.'</a>'));				
					}
				}
			}
	...
	function wizardForm_display($step)
	{
		...
		case 'save':
			...
			if (!$io_handler->exists($io))
				if (!$io_handler->insert($io))
					redirect_header('index.php?op=wizard', 2, _XT_IOSAVEFAIL);
				else
					redirect_header('index.php?op=wizard', 2, _XT_IOSAVEGOOD);				
			else
				redirect_header('index.php?op=wizard', 2, _XT_IOEXISTS);
				
			break;
		case 'finished':

			$form_sel = new XoopsThemeForm(_XT_SELECTLANGUAGE, "sellang", $_SERVER['PHP_SELF'] ."");
			$form_sel->setExtra( "enctype='multipart/form-data'" ) ;
			...
			$table_from = new XoopsFormSelect(_XT_SELECTLANGFROM.':', 'languagefrom');
			...
			$table_to = new XoopsFormSelect(_XT_SELECTLANGTO.':', 'languageto');
		...
		case '2':
			...
			$form_sel = new XoopsThemeForm(_XT_SELECTMODULEPATH, "sellang", $_SERVER['PHP_SELF'] ."");
			$form_sel->setExtra( "enctype='multipart/form-data'" ) ;
			$form_sel->addElement(new XoopsFormHidden('point',$_POST['point']));
			$form_sel->addElement(new XoopsFormHidden('module',$_POST['module']));			
			...
			$table_sel = new XoopsFormSelect(_XT_SELECTPATH.':', 'path');
		...
		case '1':
			...
			$form_sel = new XoopsThemeForm(_XT_SELECTBASEPATH, "sellang", $_SERVER['PHP_SELF'] ."");
			$form_sel->setExtra( "enctype='multipart/form-data'" ) ;
			...
			case "module":
				...
				$table_sel = new XoopsFormSelect(_XT_SELECTPATH.':', 'module');				
			}
		...
		default:
			...
			$form_sel = new XoopsThemeForm(_XT_SELECTPOINTMODE, "selpoint", $_SERVER['PHP_SELF'] ."");
			$form_sel->setExtra( "enctype='multipart/form-data'" ) ;
			
			$table_sel = new XoopsFormSelect(_XT_SELECTPOINT.':', 'select');
			$table_sel->setExtra('onchange="window.location=\'\'+this.options[this.selectedIndex].value"');
